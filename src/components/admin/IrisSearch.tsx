...
function ProjectCard({ project, isExpanded, onToggleUnits }: {
  project: IrisProject
  isExpanded: boolean
  onToggleUnits: () => void
}) {
  ...
  return (
    <div className={cn(
      'bg-white rounded-xl border overflow-hidden hover:shadow-md transition-shadow flex flex-col',
      isExpanded ? 'border-brand-primary shadow-md' : 'border-gray-200'
    )}>
      {/* Imagen */}
      <div className="relative h-44 bg-gray-100 flex-shrink-0">
        ...
        <div className="absolute top-2 left-2 flex gap-1.5 flex-wrap">
          {project.source === 'ufplus' && (
            <span className="bg-brand-secondary text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">UFPLUS</span>
          )}
          {project.pie_bonus && (
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex gap-1.5 flex-wrap mb-2">
          {tags.map((t) => (
            <span key={t} className="text-[10px] font-medium bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{t}</span>
          ))}
        </div>
        ...
        {project.units.length > 0 && (
          <button
            ...
            className={cn(
              'flex items-center gap-1.5 text-xs font-medium mt-auto transition-colors',
              isExpanded ? 'text-brand-primary' : 'text-brand-primary/70 hover:text-brand-primary'
            )}
          >
            {isExpanded ? '▲ Ocultar unidades' : `▼ Ver ${project.units.length} unidades`}
          </button>
        )}
      </div>
    </div>
...
function UnitsPanel({ project, onClose }: { project: IrisProject; onClose: () => void }) {
  ...
  const SortTh = ({ col, label, align = 'left' }: { col: SortKey; label: string; align?: 'left' | 'right' }) => (
    <th
      className={cn('px-4 py-3 font-medium cursor-pointer select-none hover:text-gray-700 whitespace-nowrap text-gray-500', align === 'right' ? 'text-right' : 'text-left')}
  ...
  return (
    <div className="bg-white rounded-xl border border-brand-primary/30 shadow-md overflow-hidden">
      {/* Header del panel */}
      <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
        <div>
          <span className="text-sm font-semibold text-gray-900">{project.title}</span>
          <span className="ml-2 text-xs text-gray-400">· {project.units.length} unidades</span>
        </div>
          ...
          <tbody className="divide-y divide-gray-50">
            {sortedUnits.map((u) => (
              <tr key={u.id} className="hover:bg-gray-50">
                <td className="px-4 py-2 text-gray-700 font-medium">{u.description || u.tipology}</td>
...
export default function IrisSearch() {
  ...
  const handleToggleUnits = (id: string) => {
              ...
              {projects.flatMap((p, i) => {
                const card = (
                  <ProjectCard
                    key={p.id}
                    project={p}
                    isExpanded={String(p.id) === expandedId}
                    onToggleUnits={() => handleToggleUnits(String(p.id))}
                  />
                )
                if (i === panelInsertAfter && expandedProject && expandedProject.units.length > 0) {
                  return [
                    card,
                    <div key="units-panel" className="col-span-full">
                      <UnitsPanel project={expandedProject} onClose={() => setExpandedId(null)} />
                    </div>,
                  ]
                }
                return [card]
